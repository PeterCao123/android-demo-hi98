 ...
 */
public class VoidAuthCompletionCommand extends AbstractCommand {
    public VoidAuthCompletionCommand(Key key) {
        super(key);
    }
    ...
    Observable<TxResponse> callService(TransactionDto dto) {
        return ServiceManager.services(getServiceTypeByIssuerCode(dto.getIssuerCode()))
                .flatMap(serviceInfo -> {
                    ...
                    return service.voidAuthCompletion(buildRequest(dto));
                });
    }
