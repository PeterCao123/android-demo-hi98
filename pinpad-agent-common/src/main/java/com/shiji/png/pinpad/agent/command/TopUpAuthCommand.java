 ...
 */
public class TopUpAuthCommand extends AbstractCommand {
    public TopUpAuthCommand(Key key) {
        super(key);
    }
    ...
    Observable<TxResponse> callService(TransactionDto dto) {
        return ServiceManager.select()
                ...
                .flatMap(serviceInfo -> {
                    ...
                    return service.topUpAuth(buildRequest(dto));
                });
    }
    ...
    void fillApprovedDto(TransactionDto dto, TxResponse response) {
        ...
        dto.setDccFlag(response.getDccFlag().getValue());
